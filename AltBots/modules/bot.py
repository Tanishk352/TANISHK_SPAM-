...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"» __𝐔sᴇʀ𝐃ɪᴇᴅ_ 𝐗_ 𝐒ᴘᴀᴍ__")
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"__🤖 ᴘɪɴɢ__KISKI MAA CHODNI HAI BOSS\n» `{mp} ᴍꜱ`")
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"» __ᴀᴅᴅɪɴɢ ᴜꜱᴇʀ ᴀꜱ  ꜱᴜᴅᴏ...__ @TANISHK352")
        target = ""
